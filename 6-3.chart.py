...
chart = alt.Chart(fi).mark_circle().encode(x='총매출', y = sel, color= '상품')



fig = alt.Chart(chart).properties(width=650, height=450)
...
with col1:
    ...
    fig = px.pie(titanic, values = 'Survived', names = sel)
    fig.update_traces(textposition='inside', textinfo = 'percent+label+value')
    #fistreamlit run streamlit\6-3.chart.pyg.update_layout()
